    ...
    for trial = 1:epoch/size_batch
        ...
        for test = 1:size_batch
            ...
            for l = 2:layers
                ...
                if isnan(sum(sum(delta_w{l})))
                    k = 1
                end
                if isnan(sum(sum(delta_theta{l})))
                    k = 1
                end
            end
        ...
        for test = 1:size_batch
            dH = dH + 0.5*(training_set_out(numbers(test),:)-O(test))*(training_set_out(numbers(test),:)-O(test))';
        end
%         if dH > 5
%             dH = 0;
%         end
        H(trial+(epochs-1)*epoch/size_batch) = dH;
    end
